...
class Rental
  ...
  def dump_rental_result()
    ...
    return Hash["id" => @id, "price" => total_price, "commission" => gen_commission(total_price)]
  end
  ...
  def gen_commission(total_price)
    comm = total_price * 0.3
    return {
             "insurance_fee": (comm /= 2).to_i,
             "assistance_fee": @duration * 100,
             "drivy_fee": (comm - (@duration * 100)).to_i,
           }
  end
end
